...
test('when sending now it should just create the campaign', function(){
    ...
    assertDatabaseCount('campaigns.create', 1);
});

test('when sending later the send at should be required', function(){
    post($this->route, ['send_when' => 'later'])
        ->assertSessionHasErrors(['sent_at' => __('validation.required', ['attribute' => 'sent at'])]);
});

test('campaign should be created when sending later', function(){
    post($this->route, ['send_when' => 'later', 'sent_at' => now()->addDays(6)])
        ->assertSessionHasNoErrors([
            'sent_at' => __('validation.after', ['attribute' => 'sent at', 'date' => 'today'])
        ]);

    assertDatabaseCount('campaigns.create', 1);
});

test('sent_at should be in the future', function(){
    post($this->route, ['send_when' => 'later', 'sent_at' => now()->subDays(6)])
