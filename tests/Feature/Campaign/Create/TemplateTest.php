...
test('when submiting the form with a body it should be redirect to the schedule tab', function(){
    post(route('campaigns.create'), ['body' => 'algum body'])
        ->assertRedirect(route('campaigns.create', ['tab' => 'schedule']));
});

test('when submiting the form with a body the session should be updated to the body information', function(){
    post($this->route, ['body' => 'algum body'])
        ->assertSessionHasNoErrors();

        expect(session('campaigns.create')['body'])->toBe('algum body');
});
